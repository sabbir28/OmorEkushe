...
// Module : platform/windows (Animated Splash Screen)
// Purpose: Displays a fade-in splash screen during application startup.
//          The splash screen is a WS_EX_LAYERED popup window that:
//            1. Loads a splash.png via stb_image and converts RGBA -> BGRA
//            2. Fades in from alpha 0 -> 255 using UpdateLayeredWindow()
//            3. Runs a user-provided initTask callback during the fade
//            4. Auto-closes after a minimum visible time (5 seconds)
//            5. Calls onCompleted callback on destruction (triggers main window show)
//
//          The SplashScreenController is heap-allocated and self-deletes on
//          WM_NCDESTROY, ensuring correct cleanup even if the window is
        ...
        private:
            ...
            void LoadBackgroundImage() {
                ...
                if (resolvedPath.empty()) {
                    MessageBoxW(nullptr, L"Required resource file 'splash.png' was not found in data directory.", L"Omor Ekushe Resource Warning", MB_OK | MB_ICONWARNING);
                    return;
                }
                ...
                if (!data) {
                    MessageBoxW(nullptr, L"Failed to load 'splash.png'. Image file may be corrupt.", L"Omor Ekushe Resource Warning", MB_OK | MB_ICONWARNING);
                    return;
                }
